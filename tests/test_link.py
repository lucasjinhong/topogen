...
def test_generate_links():
    ...
    links = generate_links(nodes, size_of_grid_lens)
    ...
    assert links[('d', '1')].data_rate_bps == DATA_RATE_BPS_FORMULA(dist_between_coord(nodes['d'].coordinate, nodes['1'].coordinate) * size_of_grid_lens)
    assert links[('d', '2')].data_rate_bps == DATA_RATE_BPS_FORMULA(dist_between_coord(nodes['d'].coordinate, nodes['2'].coordinate) * size_of_grid_lens)

    dist_formula = lambda dist: dist * 100
    links = generate_links(nodes, size_of_grid_lens, dist_formula)
